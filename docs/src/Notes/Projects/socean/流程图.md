...
```mermaid
graph TD
    A[开始训练流程] --> B[加载数据集<br>train/validation]
    B --> C[数据增强<br>旋转、缩放、平移、翻转等]
    C --> D[构建模型<br>使用 MobileNetV2 迁移学习]
    D --> E[冻结预训练层<br>设置不可训练]
    E --> F[添加全连接层<br>Dense + Dropout]
    F --> G[模型编译<br>损失函数、优化器、评估指标]
    G --> H[模型训练<br> fit，。 with callbacks]
    H --> I[EarlyStopping & ReduceLROnPlateau]
    I --> J[保存模型<br>.keras + SavedModel]
    J --> K[保存训练曲线和结构图]
    K --> L[结束训练流程]
